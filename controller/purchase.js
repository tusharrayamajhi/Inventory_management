  ...
  if (req.url == "/purchase/add" && req.method == "GET") {
    ...
    try {
      ...
      for (let purchases of body.formsdata) {
        ...
        purchase.balance =
          parseInt(purchase.ordered_qnt) - parseInt(purchase.received_qnt);
        let total = 0;
        if (purchases.vat == "13%") {
          total =
      ...
      purchaserecord.forEach(record=>{
        let value = `("${purchasecode}",${record.ordered_qnt},${record.received_qnt},${record.unit_rate},${record.vatrate.split('%')[0]},${record.balance},${record.total},"${record.status}","${record.remark}",${body.vendor.vendorid},${record.product_id},${user.id})`
        ...
        const [result] = await connection.promise().query(`INSERT INTO purchases (purchase_code,ordered_qnt, received_qnt, unit_rate, vat_rate,balance,total, status, remarks, vendor, product,user) VALUES ${values}`)
