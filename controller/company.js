...
module.exports = async function company(req, res) {
  ...
  if (req.url == "/company/add" && req.method == "GET") {
    ...
    try {
      ...
      .promise()
      .query(
        "INSERT INTO companies (company_name,registration_no,phone,email, pan_vat_no,isvat,address,country,state,city,zip,  bank_name,  account_no,website,company_logo) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
        [
            body.company_name,
